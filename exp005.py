...
class MMModel(nn.Module):
    def __init__(self, model_name, num_feat):
        super(MMModel, self).__init__()
        self.model = timm.create_model(model_name=model_name, num_classes=0, pretrained=True, in_chans=3)
        num_features = self.model.num_features
        self.emb = nn.Linear(num_feat, 128)
        self.head = nn.Sequential(nn.Dropout(0.5), nn.Linear(num_features+128, 1))

    def forward(self, x, feat):
        img_feat = self.model(x)
        table_feat = self.emb(feat)
        output = self.head(torch.cat([img_feat, table_feat], dim=1))
        return output
...
class PawpularDataset(Dataset):
    def __init__(self, df, transform=None, conf=None):
        ...
        '''
        self.dense_features = df[['file_size', 'width', 'height', 'Subject Focus', 'Eyes', 'Face', 
        ...
        'japanese_chin', 'keeshond', 'leonberger', 'maine_coon', 'miniature_pinscher', 'newfoundland', 'persian', 
        'pomeranian', 'pug', 'ragdoll', 'russian_blue', 'saint_bernard', 'samoyed', 'scottish_terrier', 'shiba_inu', 
        'siamese', 'sphynx', 'staffordshire_bull_terrier', 'wheaten_terrier', 'yorkshire_terrier', 'dog1_cat0_label', 
        'num_cats_0.9', 'num_dogs_0.9', 'num_cats_0.8', 'num_dogs_0.8', 'num_cats_0.7', 'num_dogs_0.7', 'num_cats_0.6', 'num_dogs_0.6', 
        'num_cats_0.0', 'num_dogs_0.0', 'age', 'image_area', 'dog_area_ratio', 'cat_area_ratio', 'person_area_ratio', 'center_dog_area_ratio', 
        'center_cat_area_ratio', 'center_person_area_ratio', 'breed', 'Black_prob', 'Brown_prob', 'Golden_prob', 'Yellow_prob', 'Cream_prob', 
        'Gray_prob', 'White_prob']].values#79
        '''
        self.dense_features = df[['Subject Focus', 'Eyes', 'Face', 
        'Near', 'Action', 'Accessory', 'Group', 'Collage', 'Human', 'Occlusion', 'Info', 'Blur', 
        'abyssinian', 'american_bulldog', 'american_pit_bull_terrier', 'basset_hound', 'beagle', 
        'bengal', 'birman', 'bombay', 'boxer', 'british_shorthair', 'chihuahua', 'egyptian_mau', 
        'english_cocker_spaniel', 'english_setter', 'german_shorthaired', 'great_pyrenees', 'havanese', 
        ...
        'siamese', 'sphynx', 'staffordshire_bull_terrier', 'wheaten_terrier', 'yorkshire_terrier', 'dog1_cat0_label', 
        'num_cats_0.9', 'num_dogs_0.9', 'num_cats_0.8', 'num_dogs_0.8', 'num_cats_0.7', 'num_dogs_0.7', 'num_cats_0.6', 'num_dogs_0.6', 
        'num_cats_0.0', 'num_dogs_0.0', 'age', 'dog_area_ratio', 'cat_area_ratio', 'person_area_ratio', 'center_dog_area_ratio', 
        'center_cat_area_ratio', 'center_person_area_ratio', 'breed', 'Black_prob', 'Brown_prob', 'Golden_prob', 'Yellow_prob', 'Cream_prob', 
        'Gray_prob', 'White_prob']].values#75
    def __len__(self):
        return len(self.df)
...
class SETIDataModule(pl.LightningDataModule):
    ...
    def setup(self, stage=None):
        if stage == 'fit':
            ...
            df = pd.read_csv(os.path.join('.', "train_meta.csv"))
...
class LitSystem(pl.LightningModule):
    def __init__(self, conf):
        ...
        self.model = MMModel(self.hparams.model_name, 75)
