...
class ELRLoss(nn.Module):
    def __init__(self, num_examp, num_classes=10, beta=0.3):
        ...
        self.target = torch.ones(num_examp, self.num_classes).cuda() if self.USE_CUDA else torch.zeros(num_examp, self.num_classes)
        self.target = self.target * 0.37
        self.beta = beta
        

    def forward(self, index, output, label):
        y_pred = torch.sigmoid(output) #F.softmax(output,dim=1)
        y_pred = torch.clamp(y_pred, 1e-4, 1.0-1e-4)
        y_pred_ = y_pred.data.detach()
        self.target[index] = self.beta * self.target[index] + (1-self.beta) * (y_pred_)
        ce_loss = F.binary_cross_entropy_with_logits(output, label)
        #elr_reg = ((1-(self.target[index] * y_pred).sum(dim=1)).log()).mean()
        elr_reg = (((self.target[index] - y_pred).abs())).mean()
...
class LitSystem(pl.LightningModule):
    def __init__(self, conf, num_examp):
        super().__init__()
        #self.conf = conf
        ...
        self.criteria = ELRLoss(num_examp=num_examp, num_classes=1)
    ...
    def training_step(self, batch, batch_idx):
        ...
        if torch.rand(1)[0] < 0.5: #self.current_epoch < self.hparams.epoch*1.8:
            ...
            y_hat = self.model(x)
            loss = lam * self.criteria(batch['idx'], y_hat, y) + (1 - lam) * self.criteria(batch['idx'], y_hat, y[index])
        else:
            y_hat = self.model(x)
            #loss = self.criteria(y_hat, y.view(-1, 1))
            loss = self.criteria(batch['idx'], y_hat, y)
        return loss
    ...
    def validation_step(self, batch, batch_idx):
        ...
        y_hat = self.model(x)
        #loss = self.criteria(y_hat, y.view(-1, 1))
        loss = self.criteria(batch['idx'], y_hat, y/100.0)
...
def main():
    ...
    lit_model = LitSystem(conf, num_examp=len(data_module.train_dataset))
