...
def load_pytorch_model(ckpt_name, model, ignore_suffix='model'):
    ...
    print(res)
    return model
def center_clear(image):
    h,w = image.shape[0], image.shape[1]
    width = min(h,w)
    if h>w:
        image[:int((h-width)/2),:,:] = image[:int((h-width)/2),:,:] * 0.75
        image[int((h-width)/2+width):,:,:] = image[int((h-width)/2+width):,:,:] * 0.75
    else:
        image[:,:int((w-width)/2),:] = image[:,:int((w-width)/2),:] * 0.75
        image[:,int((w-width)/2+width):,:] = image[:,int((w-width)/2+width):,:] * 0.75
    return image
####################
# Config
####################
