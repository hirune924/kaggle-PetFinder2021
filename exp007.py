...
class CustomModel(nn.Module):
    def __init__(self, model_name):
        ...
        self.head = nn.Sequential(nn.Dropout(0.5), nn.Linear(num_features, 1))
...
class PawpularDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        #targets = np.clip(targets + np.random.randint(-20,20), 0, 100)
...
class SETIDataModule(pl.LightningDataModule):
    ...
    def setup(self, stage=None):
        if stage == 'fit':
            ...
            #oof_pth = glob.glob(os.path.join('../output', 'exp006-swin_large_patch4_window12_384-bce-drop_restart',  f'oof-*.csv'))[0]
            #oof_df = pd.read_csv(oof_pth)
            #oof_df['error'] = (oof_df['Pawpularity'] - oof_df['preds']).abs()
            #oof_df = oof_df[oof_df['error']<50]
            #train_df = train_df[train_df['Id'].isin(oof_df['Id'].values)]
...
class LitSystem(pl.LightningModule):
    def __init__(self, conf):
        ...
        self.model = CustomModel(self.hparams.model_name)
        #if self.hparams.model_path is not None:
        target_ckpt_pth = glob.glob(os.path.join('../output', 'exp006-swin_large_patch4_window12_384-bce-drop_restart',  f'fold{conf.fold}/ckpt/*epoch*.ckpt'))
        print(f'load model path: {target_ckpt_pth}')
        self.model = load_pytorch_model(target_ckpt_pth[0], self.model, ignore_suffix='model')
    ...
    def configure_optimizers(self):
        ...
        optimizer = torch.optim.AdamW(self.model.head.parameters(), lr=self.hparams.lr)
        ...
        scheduler = torch.optim.lr_scheduler.CosineAnnealingWarmRestarts(optimizer, T_0=self.hparams.epoch, eta_min=self.hparams.lr*0.01)
