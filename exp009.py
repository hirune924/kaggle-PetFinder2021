...
class CustomModel(nn.Module):
    def __init__(self, model_name):
        ...
        self.head = nn.Sequential(nn.Dropout(0.5), nn.Linear(num_features, 100))
...
class LitSystem(pl.LightningModule):
    ...
    def configure_optimizers(self):
        ...
        scheduler = torch.optim.lr_scheduler.CosineAnnealingWarmRestarts(optimizer, T_0=self.hparams.epoch, eta_min=1e-6)
    ...
    def validation_epoch_end(self, outputs):
        ...
        y_hat = torch.cat([x["y_hat"].sigmoid() for x in outputs]).cpu().detach().numpy()

        #print(y.sum(axis=1))
        #print((y_hat>0.5).sum(axis=1))
        val_score = metrics.mean_squared_error(y.sum(axis=1), (y_hat>0.5).sum(axis=1), squared=False)
